...
def import_prompts(
    ...
    success_count = sum(p.get("status") == "imported" for p in imported_prompts)
    existing_count = sum(p.get("status") == "already_exists" for p in imported_prompts)
    error_count = sum(p.get("status") == "error" for p in imported_prompts)
